...
public class SegmentGeometry : UnsectionedLineGeometry, ILineSegmentGeometry
{
    ...
    public override ILength Length => LineSegmentGeometry.GetLength(this);

    public override IWindow Extent => LineSegmentGeometry.GetExtent(this);
    ...
    internal override ILength GetLength(IPosition? asFarAs)
    {
        if (asFarAs is null)
            ...
            return new Length(BasicGeom.Distance(Start, asFarAs));
    }
...
*/
    ...
    internal override Side GetSide(HorizontalRay hr)
    {
        ...
        double y = End.Y;
        if ((hy-y) > MathConstants.TINY) // y<hy
            return Side.Right;
        if ((y-hy) > MathConstants.TINY) // y>hy
            return Side.Left;
