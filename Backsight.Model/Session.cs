...
public class Session
{
    ...
    /// </summary>
    /// <param name="store">The map store the session is part of</param>
    /// <param name="sessionData">Information about the session</param>
    internal Session(IMapStore store, NewSessionEvent sessionData)
    {
        m_MapStore = store ?? throw new ArgumentNullException(nameof(store));
    ...
    {
        return $"{m_Data.When} ({m_Data.UserName})";
    }

    /// <summary>
    /// The item number that was assigned to the <see cref="NewSessionEvent"/> record that
    /// was created for this session.
    /// </summary>
    internal uint ItemNumber => m_Data.EditSequence;
    
    /// <summary>
    /// The model that contains this session
    /// </summary>
    internal CadastralMapModel MapModel => m_MapStore.Model;

    /// <summary>
    /// The store that this session is part of.
    /// </summary>
    internal IMapStore MapStore => m_MapStore;
    
    /// <summary>
    /// The login name of the user logged on for the session. 
    /// </summary>
    internal string User => m_Data.UserName;
    ...
    /// <remarks>You can only rollback operations in the latest session.</remarks>
    ...
    internal int OperationCount => m_Operations.Count;
    ...
    /// <param name="recordChange">True if the change should be recorded in the map repository (specify
    /// <c>false</c> when deserializing an allocation).</param>
    internal void AddAllocation(IdAllocation alloc, bool recordChange)
    {
        AllocationCount++;
        
        if (recordChange)
            m_MapStore.RecordChange(alloc, 1);
    }
    ...
    /// </summary>
    internal Operation[] Edits => m_Operations.ToArray();
    ...
    {
        // Save to the map repository
        var itemCount = MapStore.ItemCount + 1 - edit.EditSequence;
        MapStore.RecordChange(edit, itemCount);
    ...
    {
        // TODOl Does UpdateOperation still call this directly?
