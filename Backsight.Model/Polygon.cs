...
public class Polygon : Ring
{
    ...
    /// </summary>
    /// <param name="mapModel">The model that the polygon will be added to.</param>
    ...
    internal Polygon(CadastralMapModel mapModel, RingMetrics rm, List<Face> edge)
    ...
    /// </summary>
    internal override double AreaExcludingIslands => base.Area - TotalIslandArea;
    ...
    /// </summary>
    internal bool HasAnyIslands => IslandCount > 0;

    /// <summary>
    /// The number of islands this polygon refers to.
    /// </summary>
    public int IslandCount => m_Islands?.Count ?? 0;
    ...
    /// </summary>
    public TextFeature? Label => m_Labels is null ? null : m_Labels[0];

    /// <summary>
    /// Associated labels (may be an empty array). This should ideally contain just one label.
    /// </summary>
    public TextFeature[] Labels => m_Labels?.ToArray() ?? [];
    ...
    /// </summary>
    public int LabelCount => m_Labels?.Count ?? 0;
        ...
        ref IPosition beststart, ref IPosition bestend, ref double bestlen)
    {
        ...
        var xseg = new IntersectionFinder(MapModel.Index, seg, true);

        // Arrange intersections along the scan line.
        var xres = new IntersectionResult(xseg);
        xres.Sort(true);
    ...
    /// </summary>
    internal override Polygon? RealPolygon => this;
    ...
    /// </summary>
    internal Island[] Islands => m_Islands?.ToArray() ?? [];
}