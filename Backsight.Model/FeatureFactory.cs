...
class FeatureFactory
{
    ...
    /// </summary>
    private readonly Dictionary<string, IFeature> m_FeatureInfo = new();
    ...
    {
        m_Operation = op ?? throw new ArgumentNullException(nameof(op));
    }
    ...
    internal IEntity PointType
    {
        get
        {
            if (m_PointType is null)
                m_PointType = m_Operation.Session.MapStore.Settings.DefaultPointType;
    ...
    internal IEntity LineType
    {
        get
        {
            if (m_LineType is null)
                m_LineType = m_Operation.Session.MapStore.Settings.DefaultLineType;
    ...
    internal IEntity TextType
    {
        get
        {
            if (m_TextType is null)
                m_TextType = m_Operation.Session.MapStore.Settings.DefaultTextType;
    ...
    internal IEntity PolygonType
    {
        get
        {
            if (m_PolygonType is null)
                m_PolygonType = m_Operation.Session.MapStore.Settings.DefaultPolygonType;
    ...
    {
        if (m_FeatureInfo.TryGetValue(itemName, out var result))
            return result;
        else
            return null;
    }
    ...
    LineFeature MakeSection(DataField field, LineFeature baseLine, PointFeature from, PointFeature to)
    {
        ...
        var section = new SectionGeometry(baseLine, from, to);
    ...
    /// </summary>
    internal Feature[] CreatedFeatures => m_Operation.Features;
}