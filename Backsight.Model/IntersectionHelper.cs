...
static class IntersectionHelper
{
        ...
        , IPointGeometry othend )
    {
        ...
        if (BasicGeom.DistanceSquared(xend, othend) < BasicGeom.DistanceSquared(start, end))
        {
            test = othend;
            startLong = start;
            endLong = end;
        ...
        , ICircleGeometry circle )
    {
        ...
        if (BasicGeom.DistanceSquared(start, x1) < Constants.XYTOLSQ)
            x1 = start;
        else if (BasicGeom.DistanceSquared(end, x1) < Constants.XYTOLSQ)
            x1 = end;

        if (nx==2)
        {
            if (BasicGeom.DistanceSquared(start, x2) < Constants.XYTOLSQ)
                x2 = start;
            else if (BasicGeom.DistanceSquared(end, x2) < Constants.XYTOLSQ)
                x2 = end;
        }
        ...
        double maxangle = reft.GetAngleInRadians(end) + MathConstants.TINY;
        ...
        if (a1 < maxangle && a2 < maxangle)
        {
            if (a2 > a1 && (a2-a1) > MathConstants.PI)
            {
            ...
            if (a1 > a2 && (a1-a2) > MathConstants.PI)
            {
        ...
        , IPointGeometry othend)
    {
        ...
        double othdsq = BasicGeom.DistanceSquared(othend, centre);
        ...
        if (othend.IsCoincident(bc) || othend.IsCoincident(ec))
        {
            ...
            if (BasicGeom.DistanceSquared(midseg, centre) > minrsq)
            {
        ...
        if (nx==2 && BasicGeom.DistanceSquared(x2, trueXend) > BasicGeom.DistanceSquared(x1, trueXend))
            x1 = x2;
        ...
        double rdiff = BasicGeom.Distance(midx, centre) - radius;
        ...
        double dsqx = BasicGeom.DistanceSquared(trueXend, x1);
        ...
        , out bool isGraze )
    {
        ...
        double minrad = radius - Constants.XYTOL;
        double minrsq = minrad * minrad;
        double sdsq = BasicGeom.DistanceSquared(centre, vs);
        double edsq = BasicGeom.DistanceSquared(centre, ve);
        ...
        if (smatch && ematch)
        {
            ...
            double mdsq = BasicGeom.DistanceSquared(centre, vm);
        ...
        if (smatch || ematch)
        {
            ...
            if (nx==1)
            {
                if (BasicGeom.DistanceSquared(vp, x1) < Constants.XYTOLSQ)
                {
                    if (smatch)
                        x1 = vs;
                    else
            ...
            else
            {
                ...
                if (BasicGeom.DistanceSquared(vs, x2) < BasicGeom.DistanceSquared(vs, x1))
                {
                    IPosition temp = x1;
                    x1 = x2;
                    x2 = temp;
                ...
                if (smatch && BasicGeom.DistanceSquared(vp, x1) < Constants.XYTOLSQ)
                    x1 = vs;

                if (ematch && BasicGeom.DistanceSquared(vp, x2) < Constants.XYTOLSQ)
                    x2 = ve;
            }
        }
        else
        {
            ...
            if (nx==1)
            {
                if (BasicGeom.DistanceSquared(vs, x1) < Constants.XYTOLSQ)
                    x1 = vs;
                else if (BasicGeom.DistanceSquared(ve, x1) < Constants.XYTOLSQ)
                    x1 = ve;
            ...
            if (BasicGeom.DistanceSquared(ve, x2) < Constants.XYTOLSQ)
                x2 = ve;
        }

        // We got 2 intersections, so see if they make a graze.
        IPosition xm = Position.CreateMidpoint(x1, x2);
        double xmdsq = BasicGeom.DistanceSquared(centre, xm);
    ...
    internal static uint Intersect(IntersectionResult results, ICircularArcGeometry ab, ICircularArcGeometry pq)
    {
        ...
        if (nx==1)
        {
            ...
            if (BasicGeom.IsInSector(loc, thiscen, a, b, 0.0) &&
                BasicGeom.IsInSector(loc, othrcen, p, q, 0.0))
            {
                results.Append(loc);
                return 1;
            }
        ...
        else
        {
            ...
            if (BasicGeom.IsInSector(loc1, thiscen, a, b, 0.0) &&
                BasicGeom.IsInSector(loc1, othrcen, p, q, 0.0))
            {
                results.Append(loc1);
                nok++;
            }

            if (BasicGeom.IsInSector(loc2, thiscen, a, b, 0.0) &&
                BasicGeom.IsInSector(loc2, othrcen, p, q, 0.0))
            {
                results.Append(loc2);
                nok++;
            }
        ...
        , bool isFirstEndCoincident )
    {
        ...
        if (nx==2 && BasicGeom.DistanceSquared(x2, xend) > BasicGeom.DistanceSquared(x1, xend))
            x1 = x2;
        ...
        bool isxthis = BasicGeom.IsInSector(xloc, c1, ab.First, ab.Second, 0.0);
        if (!isxthis)
        {
            xsect.Append(xend);
            return 1;
        ...
        double rdiff = BasicGeom.Distance(midx, c1) - r1;
        if (Math.Abs(rdiff) > 0.0001)
        {
            xsect.Append(xend);
            xsect.Append(x1);
        ...
        // Given that it's a graze, assume that it's ok to work out
        // the arc distance as if it was straight.
        double dsqx = BasicGeom.DistanceSquared(xend, x1);
    ...
    static uint Intersect(IntersectionResult xsect, ICircleGeometry c, IPointGeometry start, IPointGeometry end, ICircleGeometry arcCircle)
    {
        ...
        if (nx==1)
        {
            ...
            if (BasicGeom.IsInSector(loc, centre, start, end, angtol))
            {
                xsect.Append(loc);
                return 1;
            }
        ...
        else
        {
            ...
            if (BasicGeom.IsInSector(loc1, centre, start, end, angtol))
            {
                xsect.Append(loc1);
                nok++;
            }

            if (BasicGeom.IsInSector(loc2, centre, start, end, angtol))
            {
                xsect.Append(loc2);
                nok++;
            }
