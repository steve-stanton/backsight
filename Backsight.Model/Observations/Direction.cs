...
abstract class Direction : Observation
{
	...
	internal IPosition StartPosition
	{
		get
		{
			...
			double sfac = CoordinateSystem.GetLineScaleFactor(from, approx);
...
return gotone;
	...
	{
		double result;
		for (result=a; result<0.0; result += MathConstants.PIMUL2) { }
		for (; result >= MathConstants.PIMUL2; result -= MathConstants.PIMUL2) { }
		return result;
	}
	...
	{
		if (m_Offset != null)
			editSerializer.WritePersistent<Offset>(DataField.Offset, m_Offset);
	}
