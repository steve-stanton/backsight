...
using Backsight.Model.Index.Point;
using Backsight.Model.Index.Rectangle;
...
class MapIndex : IEditMapIndex
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MapIndex"/> class
    /// with nothing in it.
    /// </summary>
    internal MapIndex()
    {
    ...
    public void Add(IMapObject o)
    {
        ...
        // Could cover this by using the fact that an IMapObject that is
    ...
    /// </returns>
    public bool Remove(IMapObject o)
    {
        if (o is IPoint p)
    ...
    {
        Extent w = extent is null || extent.IsEmpty ? new Extent() : new Extent(extent);
