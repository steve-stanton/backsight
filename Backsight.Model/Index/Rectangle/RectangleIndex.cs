...
class RectangleIndex : Node
{
    ...
    {
        return m_Children?.FirstOrDefault(n => w.IsEnclosedBy(n.Window));
    }
    ...
    /// </summary>
    internal override bool IsEmpty => this.ItemCount == 0 && this.ChildCount == 0;

    /// <summary>
    /// The number of child nodes associated with this node.
    /// </summary>
    uint ChildCount => m_Children==null ? 0 : (uint)m_Children.Count;
}