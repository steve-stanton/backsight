...
class Node
{
    ...
    internal virtual void Query(Extent searchExtent, ProcessItem itemHandler)
    {
        ...
        foreach (Item i in m_Items)
        {
            ...
            {
                bool keepGoing = itemHandler(i.MapObject);
                if (!keepGoing)
                    return;
            }
        }
    ...
    protected bool RemoveItem(Item item)
    {
        ...
        foreach (Item i in m_Items)
        {
            if (ReferenceEquals(i.MapObject, item.MapObject))
            {
                m_Items.Remove(i);
                if (m_Items.Count == 0)
                    m_Items = null;
    ...
    /// </summary>
    internal uint ItemCount => m_Items is null ? 0 : (uint)m_Items.Count;

    /// <summary>
    /// Does this indexing node contain any items (checks whether
    /// the <see cref="ItemCount"/> property has a value of 0).
    /// </summary>
    internal virtual bool IsEmpty => this.ItemCount==0;
}