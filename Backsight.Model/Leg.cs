...
abstract class Leg
{
    ...
    internal abstract Circle Circle { get; }
    internal abstract ILength Length { get; }
    internal abstract IPosition Center { get; }
    internal abstract void Project(ref IPosition pos, ref double bearing, double sfac);

    /// <summary>
    /// Gets the geometry for spans along this leg (to be called only via implementations of <see cref="GetSections"/>).
    /// </summary>
    ...
    internal abstract ILineGeometry[] GetSpanSections(IPosition start, double bearing, double sfac, SpanInfo[] spans);
    ...
    /// <param name="spans">Information for the spans coinciding with this leg.</param>
    /// <returns>The sections along this leg</returns>
    internal abstract ILineGeometry[] GetSpanSections(IPosition start, IPosition end, SpanInfo[] spans);
    ...
    internal abstract LineFeature CreateLine(FeatureFactory ff, string itemName, PointFeature from, PointFeature to);
    ...
    /// underlying line)?
    /// </summary>
    internal bool IsStaggered => m_OtherSide is not null;
    ...
    /// spans relate to both sides of the leg).
    /// </summary>
    internal LegFace PrimaryFace => m_FirstSide;
