...
class PolygonBuilder
{
    ...
    bool ProcessLine(IMapObject o)
    {
        ...
        {
            string msg = $"Error building polygon starting at line {line} ({e.Message})";
    ...
    /// <param name="o">An item found in the spatial index.</param>
    /// <returns>True (always), indicating that the spatial query should keep going.</returns>
    bool ProcessPolygon(IMapObject o)
    {
        if (o is not Island pol)
            return true;

        // Just return if the enclosing polygon is already known
        if (pol.Container is not null)
            return true;
    ...
    /// <param name="o">An item found in the spatial index.</param>
    /// <returns>True (always), indicating that the spatial query should keep going.</returns>
    bool ProcessLabel(IMapObject o)
    {
    ...
    bool FindLabelForPolygon(IMapObject o)
    {
        ...
        if (o is Polygon p)
        {
            ...
            if (p.Label is null && !p.HasAnyIslands)
            {
