...
class PolygonLink
{
    ...
    /// </summary>
    internal bool IsCurveEnd => m_IsCurveEnd;
    ...
    {
        double cornerTolerance = 20.0 * MathConstants.DEGTORAD;
    ...
    {
        return m_Angle > (MathConstants.PIDIV2 - tol) && m_Angle < (MathConstants.PIDIV2 + tol);
    }
    ...
    /// </summary>
    internal bool IsLinked => m_Link is not null;
    ...
    /// </summary>
    PointFeature Point => m_Point;

    /// <summary>
    /// Is this a radial link?
    /// </summary>
    internal bool IsRadial => m_IsRadial;
    ...
    internal void Define(PointFeature point, bool isCurveEnd, bool isRadial, double bearing, double angle)
    {
        ...
        {
            m_Angle = angle;
            if (m_Angle > MathConstants.PI)
                m_Angle -= MathConstants.PI;
        }

        // Important that the bearing is in range [0,PIMUL2), for use with Turn.GetAngle
        m_Bearing = bearing;
        if (m_Bearing >= MathConstants.PIMUL2)
            m_Bearing -= MathConstants.PIMUL2;
    }
    ...
    {
        SetLink(link, MathConstants.PIMUL2, MathConstants.PIMUL2);
    }
    ...
    internal void SetLink(PolygonLink link, double thisdiff, double othrdiff)
    {
        ...
        if (m_Link!=null)
        {
            m_Link.Link = null;
            m_Link.IsEnd = false;
            m_Link.LinkAngle = MathConstants.PIMUL2;
        }
    ...
    internal double GetAngle(PolygonLink other)
    {
        ...
        {
            angle = MathConstants.PIMUL2 - angle;
    ...
    internal bool IsLinkValid(PolygonLink other, Polygon pol)
    {
        ...
        var xsect = new IntersectionFinder(pol.MapModel.Index, seg, true);
