...
class PolygonFace
{
    ...
    /// </summary>
    IDivider Divider => m_Divider;

    /// <summary>
    /// The polygon involved
    /// </summary>
    internal Polygon Polygon => m_Polygon;
    ...
    int SetExtraPoints()
    {
        ...
        var index = m_Divider.Line.MapModel.Index;
    ...
    internal PolygonLink[] CreateLinks(PolygonFace prev)
    {
        ...
        // Try to get a point feature at the beginning of this face.
        var index = m_Polygon.MapModel.Index;
        ...
        if (isPrevCurve && isThisCurve)
        {
            ...
            if (!(prevcen==thiscen && Math.Abs(prevrad-thisrad) < MathConstants.TINY))
            {
            ...
            if (isRadial)
            {
                ...
                bearing = BasicGeom.BearingInRadians(centre, m_Begin);
                angle = 0.0;
                if (iscw  != isPolLeft)
                    bearing += MathConstants.PI;
            }
        }
        ...
        if (!isRadial)
        {
            ...
            // angle.
            var reference = new Turn(m_Begin, prev.TailReference);
            ...
            bearing = reference.BearingInRadians + angle*0.5 + MathConstants.PI;
        }

        // Initialize the link at the start of the face
        var links = new List<PolygonLink>();
        var link = new PolygonLink(beginPoint, isCurveEnd, isRadial, bearing, angle);
        links.Add(link);
        ...
        if (m_ExtraPoints!=null)
        {
            ...
            for (uint i=0; i<m_ExtraPoints.Length; i++)
            {
                ...
                {
                    Debug.Assert(centre!=null);
                    bearing = BasicGeom.BearingInRadians(centre, loc);
                    if (iscw  != isPolLeft)
                        bearing += MathConstants.PI;
                }
                else
