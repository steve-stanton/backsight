...
class FindIntersectionsQuery
{
    ...
    internal FindIntersectionsQuery(IMapIndex index, LineFeature line, bool wantEndEnd)
    {
        if (index is null)
    ...
    /// <param name="wantEndEnd">Specify true if you want end-to-end intersections in the results.</param>
    internal FindIntersectionsQuery(IMapIndex index, LineGeometry geom, bool wantEndEnd)
    {
        m_Feature = null;
    ...
    /// </summary>
    /// <param name="index">The spatial index to search</param>
    private void FindIntersections(IMapIndex index)
    {
    ...
    private bool OnQueryHit(IMapObject item)
    {
        ...
        if (ReferenceEquals(m_Feature, f))
            return true;
    ...
    /// </summary>
    internal List<IntersectionResult> Result => m_Result;
}