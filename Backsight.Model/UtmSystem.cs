...
public class UtmSystem : ISpatialSystem
{
	/// <summary>
	...
	/// </summary>
	/// <param name="zone">The UTM zone number (1-60)</param>
	/// <param name="meanElevation">The mean elevation in the mapping area (in meters)</param>
	/// <param name="geoidSeparation">The geoid separation in the mapping area (in meters)</param>
	public UtmSystem(byte zone = 14, double meanElevation = 0.0, double geoidSeparation = 0.0)
	{
		...
		m_MeanElevation = new Length(meanElevation);
		m_GeoidSeparation = new Length(geoidSeparation);
	}

	public override string ToString()
	{
		return $"{m_Projection}.{m_Zone} ({m_Ellipsoid})";
	}
	...
	public byte Zone
	{
		...
		internal set
		{
			if (m_Zone<=0 || m_Zone>60)
				throw new ArgumentOutOfRangeException(nameof(Zone), "UTM zone number must be in range [1,60]");
	...
	{
		return (-183.0 + ((double)zoneNumber * 6.0)) * MathConstants.DEGTORAD;
	}
	...
	public double GetGroundArea(IPosition[] v)
	{
		...
		for ( int i=1; i<v.Length; i++, f1=f2, xs=xe, ys=ye )
		{
			...
			if ( Math.Abs(extra)<MathConstants.TINY ) extra = 0.0;
			area += extra;
		}
	...
	public string Name => $"UTM {this.Zone}N - NAD83";

	public int EpsgNumber => 0;
