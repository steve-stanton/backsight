...
class PathParser
{
    ...
    internal static PathItem[] GetPathItems(string str, DistanceUnit entryUnit)
    {
        PathParser pp = new PathParser(str, entryUnit);
        return pp.m_Items.ToArray();
    }

    internal static Leg[] CreateLegs(string str, DistanceUnit entryUnit)
    {
    ...
    /// <summary>
    /// The data entry units (may change while parsing). 
    /// </summary>
    ...
    void ParseWord(string str)
    {
        ...
        if (str.Contains("..."))
        {
            DistanceUnit? unit = GetUnits(str);
            if (unit is null)
                throw new ApplicationException($"Cannot determine units in  '{str}'");
            
            // Make it the default for the rest of the data entry string
            m_Units = unit;
        ...
        {
            // Start out by assuming the current distance units.
            DistanceUnit unit = m_Units;
            ...
            {
                string msg = $"Malformed value '{str}'";
                throw new ApplicationException(msg);
            }
            ...
            {
                unit = GetUnits(str.Substring(num.Length));
                if (unit is null)
                {
                    string msg = $"Malformed value '{str}'";
                    throw new ApplicationException(msg);
    ...
    /// match one of the abbreviations for the desired units.</param>
    /// <returns>The corresponding units (null if the units could not be parsed).</returns>
    DistanceUnit? GetUnits(string str)
    {
        ...
        // Try to match the abbreviation to one of the known unit types
