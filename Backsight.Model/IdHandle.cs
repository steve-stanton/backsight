...
class IdHandle
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="IdHandle"/> class that doesn't refer to anything.
    /// </summary>
    /// <param name="session">The current editing session.</param>   
    public IdHandle(Session session)
    {
        m_Session = session;
        Reset();
    }
    ...
    /// <summary>
    /// Reserves a feature ID. Any ID previously reserved by this ID handle will be released.
    /// </summary>
    ...
    internal bool ReserveId(IEntity ent, uint id)
    {
        ...
        IdManager idMan = m_Session.MapModel.IdManager;
        ...
        if (id != 0)
            throw new ArgumentException($"Failed to reserve ID {id} for '{ent.Name}'");
