...
abstract class Topology : IEnumerable<IDivider>
{
    ...
    {
        return d.Left is not null && d.Right is not null;
    }
    ...
    public abstract IEnumerator<IDivider> GetEnumerator();
    ...
    /// </summary>
    internal abstract IDivider FirstDivider { get; }

    /// <summary>
    /// The divider at the end of the associated line.
    /// </summary>
    internal abstract IDivider LastDivider { get; }
    ...
    /// </summary>
    /// <param name="mapModel">The model to which any polygons will be added.</param>
    ...
    internal void BuildPolygons(CadastralMapModel mapModel, Window bwin, EditingIndex index)
    {
        foreach (IDivider d in this)
        {
            if (!d.IsOverlap)
            {
                if (d.Left==null)
                    d.Left = BuildSide(mapModel, d, bwin, index, true);

                if (d.Right==null)
                    d.Right = BuildSide(mapModel, d, bwin, index, false);
            }
        }
    }

    Ring BuildSide(CadastralMapModel mapModel, IDivider d, Window bwin, EditingIndex index, bool isLeft)
    {
        List<Face> edge = GetPolygonFaces(d, isLeft);
        Ring result = Ring.Create(mapModel, edge);
    ...
    List<Face> GetPolygonFaces(IDivider start, bool isPolLeft)
    {
        ...
        for (; ; )
        {
            ...
            var connect = new ConnectionFinder();
    ...
    /// </summary>
    internal abstract void OnLineDeactivation();
