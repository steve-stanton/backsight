...
class ParallelLineOperation : Operation, IRecallable, IRevisable, IFeatureRef
{
    ...
    internal LineFeature ReferenceLine => m_RefLine;
    ...
    internal Observation Offset => m_Offset;

    /// <summary>
    /// The point (if any) that was used to define the offset to the parallel line.
    /// </summary>
    internal PointFeature OffsetPoint => (m_Offset as OffsetPoint)?.Point;

    internal bool IsArcReversed => (m_Flags==1);
    internal LineFeature Terminal1 => m_Term1;
    internal LineFeature Terminal2 => m_Term2;
    ...
    internal override bool Rollforward()
    {
        ...
        if (circle!=null)
        {
            ...
            double parrad = BasicGeom.Distance(circle.Center, spar);
            if (Math.Abs(circle.Radius - parrad) > MathConstants.XYRES)
            {
    ...
    internal override EditingActionId EditId => EditingActionId.Parallel;
    ...
    internal void Execute()
    {
        ...
        IEntity pointType = Session.MapStore.Settings.DefaultPointType;
    ...
    PointFeature AddPoint(IPosition loc)
    {
        ...
        var e = Session.MapStore.Settings.DefaultPointType;
        p = MapModel.AddPoint(p, e, this);
        p.SetNextId();
        ...
        , out IPosition? epar )
    {
        ...
        if (!refLine.CalculateParallel(offset, out spar, out epar))
            return false;
        ...
        {
            spar = refLine.GetIntersect(spar, term1);
            if (spar is null)
        ...
        {
            IPosition? tpar = refLine.GetIntersect(epar, term2);
            if (tpar is null)
                //epar = ParallelLineUI.GetIntersect(refLine, epar, term2);
                //if (epar == null)
            {
                epar = refLine.GetIntersect(epar, term2);
                throw new Exception("Parallel does not intersect terminal line.");
            }
            else
            {
