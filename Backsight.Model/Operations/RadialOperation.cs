...
class RadialOperation : Operation, IRecallable, IRevisable
{
    ...
    /// </summary>
    internal PointFeature From => (m_Direction==null ? null : (PointFeature)m_Direction.From);
    ...
    /// an <c>OffsetPoint</c>).
    /// </summary>
    internal Observation Length => m_Length;
    ...
    /// </summary>
    public override string Name => "Sideshot";

    internal override EditingActionId EditId => EditingActionId.Radial;
    ...
    internal static IPosition Calculate(Direction dir, Observation len)
    {
        ...
        IPosition to = BasicGeom.Polar(start, bearing, length);
        ...
        double sfac = CoordinateSystem.GetLineScaleFactor(start, to); 
        return BasicGeom.Polar(start, bearing, length * sfac);
    }
