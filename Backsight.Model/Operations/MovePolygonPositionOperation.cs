...
class MovePolygonPositionOperation : Operation
{
        ...
        : base(label.MapStore)
    {
        if (!label.IsTopological)
            throw new ArgumentException("Text is not tagged as a polygon label");
    ...
    /// </summary>
    public override string Name => "Move polygon reference position";
    ...
    /// </summary>
    internal override EditingActionId EditId => EditingActionId.MovePolygonPosition;
    ...
    internal void Execute(PointGeometry to)
    {
        ...
        if (polPos is null || txtPos.IsCoincident(polPos))
            m_OldPosition = null;
        else
    ...
    /// The polygon label that was modified
    /// </summary>
    internal TextFeature Label => m_Label;
