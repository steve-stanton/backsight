...
class IntersectDirectionAndLineOperation : IntersectOperation, IRecallable, IRevisable, IFeatureRef
{
    ...
    double GetMaxLength(double defaultLength = 100000.0)
    {
        var extent = MapModel.Extent;
        if (extent.IsEmpty)
            return defaultLength;
        
        return BasicGeom.Distance(extent.Min, extent.Max);
    }
    ...
    internal override void CalculateGeometry(EditingContext ctx)
    {
        ...
        {
            double d = BasicGeom.Distance(p, CheckPosition);
            if (d > 1.0)
    ...
    internal LineFeature LineBeforeSplit => m_LineA;

    internal LineFeature LineAfterSplit => m_LineB;
