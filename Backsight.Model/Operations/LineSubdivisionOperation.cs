...
class LineSubdivisionOperation : Operation, IRecallable, IRevisable
{
        ...
        : base(editDeserializer)
    {
        ...
        if (editDeserializer.IsNextField(DataField.OtherSide))
        {
            ...
            Debug.Assert(OtherSide is not null);
            OtherSide.OtherSide = this;
        }

        var store = editDeserializer.MapStore;
        IEntity pointType = editDeserializer.ReadEntity(DataField.PointType);
        FeatureStub[] sections = CreateStubs(store, pointType, m_Line.EntityType);

        var result = new DeserializationFactory(this, sections);
    ...
    /// </summary>
    /// <param name="store">The map the stubs relate to.</param>
    /// <param name="pointType">The entity type for created points</param>
    /// <param name="lineType">The entity type for created lines</param>
    /// <returns></returns>
    FeatureStub[] CreateStubs(IMapStore store, IEntity pointType, IEntity lineType)
    {
        var stubList = new List<FeatureStub>();
        uint sequence = this.EditSequence;
    ...
    {
        var iid = new InternalIdValue(itemSequence);
    ...
    /// The line that was subdivided.
    /// </summary>
    internal LineFeature Parent => m_Line;
    ...
    internal void Execute()
    {
        ...
        FeatureStub[] stubs = CreateStubs(MapStore, MapStore.Settings.DefaultPointType, m_Line.EntityType);
    ...
    internal override EditingActionId EditId => EditingActionId.LineSubdivision;
    ...
    /// </summary>
    internal LineSubdivisionFace Face => m_Face;
    ...
    public override void WriteData(EditSerializer editSerializer)
    {
        ...
        if (OtherSide is not null)
            editSerializer.WriteInternalId(DataField.OtherSide, new InternalIdValue(OtherSide.EditSequence));

        IEntity pointType = MapStore.Settings.DefaultPointType;
        editSerializer.WriteEntity(DataField.PointType, pointType);
