...
public class EditDeserializer
{
    ...
    {
        var result = new Dictionary<string, ConstructorInfo>();
    ...
    /// <summary>
    /// The map that's being loaded (not null).
    /// </summary>
    readonly MapStore m_Store;
    ...
    /// </summary>
    /// <param name="store">The map that's being loaded</param>
    ...
    /// <remarks>
    /// The deserialization of a map involves the following:
    /// <list type="bullet">
    /// <item>Load the operations used to construct the map from the files present in the map folder.</item>
    /// <item>Process the operations to create the basic map features (points, lines, and polygon labels).</item>
    /// <item>Retrieve any associated attributes from the environment database and attach to the corresponding features.</item>
    /// <item>Build a spatial index.</item>
    /// <item>Break lines into sections if they intersect other lines.</item>
    /// <item>Create polygons by looking for line sections that form closed shapes.</item>
    /// <item>Associate each polygon label with their enclosing polygon.</item>
    /// </list>
    /// </remarks>
    internal EditDeserializer(MapStore store)
    {
        m_Store = store ?? throw new ArgumentNullException();
        m_CurrentEdit = null;
    ...
    /// The map that's being loaded (not null).
    /// </summary>
    internal MapStore MapStore => m_Store;

    /// <summary>
    /// The map model that deserialized data is loaded into (not null).
    /// </summary>
    internal CadastralMapModel MapModel => (m_Store as IMapStore).Model;
    ...
    /// <summary>
    /// Applies any forward references detected during prior calls to <see cref="ReadFeatureRef"/>
    /// </summary>
