...
class HorizontalRay
{
	...
	internal IPointGeometry Start => m_Start;

	internal IPointGeometry End => new PointGeometry(m_EndX, m_Start.Y);

	internal double StartX => m_Start.X;

	internal double EndX => m_EndX;

	internal double Y => m_Start.Y;
	...
	internal bool Intersect(IPosition vs, IPosition ve, ref IPosition xsect)
	{
		...
		if (Math.Abs(dy) < MathConstants.TINY )
		{
	...
	/// </summary>
	/// <returns></returns>
	internal bool IsValid => (m_EndX - m_Start.X) >= 0.0;

	public override string  ToString()
	{
		return $"{m_Start.Y}N,{m_Start.X}E -> {m_EndX}E";
	}
}