...
public class ArcGeometry : UnsectionedLineGeometry, ICircularArcGeometry, IFeatureRef
{
    ...
    public IPointGeometry BC => Start;

    public IPointGeometry EC => End;
    ...
    public override ILength Length => CircularArcGeometry.GetLength(this, null);

    /// <summary>
    /// The circle the arc falls on.
    /// </summary>
    public ICircleGeometry Circle => m_Circle;

    /// <summary>
    /// The bearing from the center of circle to the start of the arc
    /// </summary>
    public double StartBearingInRadians => CircularArcGeometry.GetStartBearingInRadians(this);

    /// <summary>
    /// The angular length of the arc
    /// </summary>
    public double SweepAngleInRadians => CircularArcGeometry.GetSweepAngleInRadians(this);

    public IPointGeometry First => CircularArcGeometry.GetFirstPosition(this);

    public IPointGeometry Second => CircularArcGeometry.GetSecondPosition(this);

    public override IWindow Extent => CircularArcGeometry.GetExtent(this);
    ...
    internal override IPosition GetOrient(bool fromStart, double crvDist)
    {
        ...
        double bearing = BasicGeom.BearingInRadians(centre, loc);
        ...
        return BasicGeom.Polar(centre, bearing, radius);
    }

    /*
     TODO: This applies only when rendering so should probably be part of a line renderer class
     
    /// <summary>
    /// Gets the annotation for this line.
    /// </summary>
...
//        }
        ...
        IPosition p = BasicGeom.Polar(center, bearing, m_Circle.Radius + offset);
    ...
    */
    ...
    /// </summary>
    internal bool IsCircle => BC.IsCoincident(EC);
    ...
    internal override Side GetSide(HorizontalRay hr)
    {
        ...
        if (result==Side.Right)
        {
            ...
            if (Math.Abs(maxnorth - hr.Y) < MathConstants.TINY)
                result = Side.Left;
        }
    ...
    internal override IPosition GetClosest(IPointGeometry p, ILength tol)
    {
        ...
        double dist = BasicGeom.Distance(center, p);
        ...
        {
            double bearing = BasicGeom.BearingInRadians(center, p);
            return BasicGeom.Polar(center, bearing, radius);
        }

        double d1 = BasicGeom.DistanceSquared(p, BC);
        double d2 = BasicGeom.DistanceSquared(p, EC);
    ...
    internal override void SetSortValues(List<IntersectionData> data)
    {
        ...
        //double angtol = MathConstants.XYTOL/radius;
        ...
        foreach (IntersectionData xd in data)
        {
            ...
            if (end.GetAngleInRadians(xpos, angtol) < MathConstants.TINY)
    ...
    internal override double GetRotation(IPointGeometry pos)
    {
        ...
        IPosition center = m_Circle.Center;
        double bearing = BasicGeom.BearingInRadians(center, pos);
